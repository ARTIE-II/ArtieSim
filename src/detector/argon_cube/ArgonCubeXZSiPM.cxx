...
namespace Artie
{
    void reverse(double* a, const int n) {
	for (int i = 0; i < n / 2; i++) {
		double tmp = a[i];
		a[i] = a[n - i - 1];
		a[n - i - 1] = tmp;
    ...
    , mPosition(Position)
    {
		...
		G4double lambda[kSamples] = { 
			0.1984, 0.2066, 0.2066, 0.2073, 0.208, 0.2087, 0.2094, 0.2101, 0.2109, 0.2116,
		...
		G4double real[kSamples] = { 
			0.968, 1.11, 1.01, 1.036, 1.046, 1.066, 1.07, 1.083, 1.088, 1.102, 1.109, 1.119, 1.24,
		...
		G4double imaginary[kSamples] = { 
			2.89, 3.05, 2.909, 2.928, 2.944, 2.937, 2.963, 2.982, 2.987, 3.005, 3.015, 3.025,
		...
		G4double absolute[kSamples];
		// Reverse arrays.
		reverse(real, kSamples);
		reverse(imaginary, kSamples);
		// Derive energy in ascending order.
		G4double energies[kSamples];
        ...
        mMaterial.reset(silicon);

		mQuantumEfficiency = new G4PhysicsOrderedFreeVector();

		std::ifstream datafile;
		datafile.open("VUV4-50ump.dat");
		while(1)
		{
			G4double wavelength, efficiency;
			datafile >> wavelength >> efficiency;
			if(datafile.eof()) {
				break;
			}
			mQuantumEfficiency->InsertValues(wavelength, efficiency);
		}
		datafile.close();
    }
    ...
    void ArgonCubeXZSiPM::Construct()
    {
        ...
        SetSolidVolume(
            new G4Box(
                "Solid_SiPM_"+std::to_string(mSiPMID), 
                mCubeX, 
                mCubeY, 
                mCubeZ
            )
        ...
        SetLogicalVolume(
            new G4LogicalVolume(
                ...
                mMaterial.get(), 
                "Logical_SiPM_"+std::to_string(mSiPMID)
            )
        );
        SetPhysicalVolume(
            new G4PVPlacement(
                ...
                "Physical_SiPM_"+std::to_string(mSiPMID), 
                GetMotherLogical(), 
                false, 
                mSiPMID, 
                true
    ...
    G4bool ArgonCubeXZSiPM::ProcessHits(G4Step* step, G4TouchableHistory* history)
    {
		G4int particle_pdg = step->GetTrack()->GetParticleDefinition()->GetPDGEncoding();
		if(abs(particle_pdg) != 22) {
			return false;
		}
		G4ThreeVector momentum = step->GetPreStepPoint()->GetMomentum();
		G4double wavelength = (1239.841939 * eV / momentum.mag());
