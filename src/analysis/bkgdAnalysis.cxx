...
namespace Artie
{
    ...
    void bkgdAnalysisFunctionEventEnd()
    {
        ...
        for (auto primary:primaries)
        {
            ...
            // Fill Neutron Energy and neutron profile
            bkgdTuple.n_energy = trajectory.E[0];
            bkgdTuple.x_start = x_start;
            bkgdTuple.y_start = y_start;
            AnalysisManager->FillNtupleDColumn(index, 0, bkgdTuple.n_energy);
            AnalysisManager->FillNtupleDColumn(index, 2, bkgdTuple.x_start);
            AnalysisManager->FillNtupleDColumn(index, 3, bkgdTuple.y_start);

            for (G4int i = 0; i < trajectory.size(); i++)
            {
                // start points
